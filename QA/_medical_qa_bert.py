...
if __name__ == '__main__':
    ...
    df_train = pd.DataFrame({'Train_text': train_text, 'choice_A': train_choices_A, 'answer': label_A})
    ...
    # df_test2 = pd.DataFrame({'Test_text': test_text, 'choice_A': test_choices_B})
    # df_test3 = pd.DataFrame({'Test_text': test_text, 'choice_A': test_choices_C})
    ...
    torch.save(model, "old_qa.pt")

    import csv

    with open('unpreprocess_qa2.csv', 'w', newline='') as csvfile:
        # 建立 CSV 檔寫入器
